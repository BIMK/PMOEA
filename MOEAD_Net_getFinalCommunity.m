function  MOEAD_Net_getFinalCommunity(path,name,real_path,overlapping,isRealWorld,c)
%%%%%
global edgeMatrix 
global AdjMatrix 
global edgeNum
     ...
     for i = 1:len
        ...
        if sum(Datalabel) ~= 0
         ...
         errorNodes=find(A(:,2)>1);
          ...
          [myLabel,remove]=find_error(single(myLabel),AdjMatrix,finalCommunity{i},qqq,degree,edgeNum);
...
function [ParetoFront1,remove]=find_error(ParetoFront1,AdjMatrix,CLique,Q,degree,edgeNum)
        M=[];
        V=length(CLique);
        ...
        for in=1:size(ParetoFront1,1)
            ...
            QQ(in) = modularity(ParetoFront1(in,1:numVar),AdjMatrix,degree,edgeNum);
            if QQ(in)>Q(in)                                                       %有疑问
                M=[M in];
            end
        end
