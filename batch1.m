%==========================================================
% MATLAB job submission script: batch1.m
%==========================================================
%% Set Workers
cores = 50;    %设置作业请求的核数
...
c.SubmitArguments = '--ntasks-per-node=10'   %添加调度系统作业提交参数,--ntasks-per-node参数限制每节点的任务数
fprintf('Creating the cluster: %.2f sec\n', toc);

%% Submit Job
tic;
j = batch(c, @run_main, 1, {50}, 'pool', workers)    %batch参数，1、并行池 2、提计算的函数 3、函数的输出参数个数 4、函数的输入参数 5、并行池参数 6、子任务数
