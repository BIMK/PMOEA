function node_code = edgeCode2nodeCode(edgeCode,edgeAdjMatrix,edgeMatrix,AdjMatrix,similarity,M)
% labelSet = unique(edgeCode);
% for i = 1:size(edgeCode,2)
%     index = find(edgeCode==labelSet(4));%某个标签对应边集合矩阵的索引
%     adjEdge = find(edgeAdjMatrix(index,:)==1);%该标签对应的边的邻接边索引
%     adjEdge2node = edgeMatrix(adjEdge,:); %邻接边对应的 
% a= 0;
global  overlapNode
...
%                if (size(intersect_node,2)/size(union_node,2))>=0.8 %重合度比较高
%                    labels_nodeSet(labels_nodeSet_i,union_node) = union_node;%向相似度大的合并
%                    labels_nodeSet(labels_nodeSet_j,:) = 0;%另一标签节点清空
%                    elseif (size(intersect_node,2)/size(union_node,2))<=0.045&&size(intersect_node,2)>0&&size(intersect_node,2)<2%检测重叠点
%                             overlapNode = unique([overlapNode,intersect_node]);
%                    elseif labels_advSim(labels_nodeSet_i,1)>labels_advSim(labels_nodeSet_j,1)%比较两个标签的平均相似度
